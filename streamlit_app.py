...
def fetch_realtime_weather():
    ...
    try:
        ...
        weather_data = {
            ...
            'tempmin': data['current']['temp_c'] - 2,  # Approximation as current doesn't have min/max
            'tempmax': data['current']['temp_c'] + 2,  # Approximation 
        ...
        while current_date <= end_date:
            ...
            try:
                ...
                for hour_data in data['forecast']['forecastday'][0]['hour']:
                    ...
                    hour_weather = {
                        'datetime': hour_time,  # Already in proper format with time component
                        'temp': hour_data['temp_c'],
                        'tempmin': day_data['mintemp_c'],  # Same daily min for all hours
                        'tempmax': day_data['maxtemp_c'],  # Same daily max for all hours
        ...
        for day in data['forecast']['forecastday']:
            ...
            for hour in day['hour']:
                hourly_forecast_data.append({
                    'datetime': hour['time'],  # Already includes time component
                    'temp': hour['temp_c'],
...
def clean_data(df):
    ...
    # Replace obvious errors with NaN
    for col in numeric_cols:
        # Replace negative values where they don't make sense
        if col in ['temp', 'tempmin', 'tempmax', 'humidity', 'precip', 'visibility', 'uv']:
            df_clean[col] = df_clean[col].apply(lambda x: np.nan if (x < -50 or x > 100) else x)  # Extreme outliers
    ...
    for col in numeric_cols:
        ...
        # Use different methods based on the column
        if col in ['temp', 'tempmin', 'tempmax']:
            # For temperature, use linear interpolation if possible
            df_clean[col] = df_clean[col].interpolate(method='linear', limit_direction='both')
        elif col in ['humidity', 'cloudcover']:
            # For percentage values, use forward fill then median
            df_clean[col] = df_clean[col].fillna(method='ffill').fillna(method='bfill').fillna(df_clean[col].median())
        else:
            # For other values, use median
            df_clean[col] = df_clean[col].fillna(df_clean[col].median())
    
    # Make sure all numeric values are valid (deal with any remaining NaNs)
    for col in numeric_cols:
        if df_clean[col].isna().any():
            # If there are still NaNs, fill with median
...
def load_or_fetch_data(initial_load=False):
    ...
    try:
        if os.path.exists("coimbatore_weather_data.csv"):
            ...
            # Handle any problematic dates
            mask = pd.isna(df['datetime'])
            if mask.any():
                # Try with flexible format for problematic rows
                df.loc[mask, 'datetime'] = pd.to_datetime(df.loc[mask, 'datetime'], format='mixed', errors='coerce')
